...
function drawline(max,color,type,diff){
    ...
    for(i=0; i<=29; i++){
        ctx.moveTo(width/10+i*22.925,margin*6+(type[i]-max)*-height*14/24/diff);
        ctx.lineTo(width/10+(i+1)*22.925,margin*6+(type[i+1]-max)*-height*14/24/diff);
    }
    ctx.lineWidth = 2;
